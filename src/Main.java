...
*/
        ...
        //- интерфейсы содержат только объявления методов (абстрактные методы) и константы (статические и финальные переменные)
        //- не могут иметь конструкторов, так как они не могут быть инстанциированы (нельзя создать экземпляр интерфейса)
        //- класс может реализовывать несколько интерфейсов, что позволяет обойти отсутствие множественного наследования в Java
        //- класс имплементирующий интерфейс подписывает контракт на реализацию всех его методов

        //константы:
        //•	поля должны быть проинициализированы
        //•	поля по умолчанию считаются публичными статическими финальными	(модификаторы public, static и final не нужно указывать явно, они «проставляются» по умолчанию)
...
//        }
        ...
        //приватные методы:
//        С появлением Java 8 и введением методов по умолчанию и статических методов, у интерфейсов появилась возможность
//        содержать не только сигнатуры методов, но и их реализации. При написании таких реализаций рекомендуется
//        разделять сложные методы на более простые. Такой код легче переиспользовать, поддерживать и понимать.
...
//        }
        ...
        System.out.println("Ниже по видео");
        Dog dog = new Dog();
        Fish fish = new Fish();
        Bird bird = new Bird();
        ...
        Animal animal = new Dog();  //приведение типа собаки к родительскому происходить автоматически - upcast

        Dog dog1 = new Dog();
        dog1.run();
        Animal animal1 = dog1;  //приведение типа - upcast
        //animal.run();   //тут у нас будет ошибка, тк у нас более нет доступа к методу run() в классе Dog, тк какой тип
                          //туда и доступ
        dog1 = (Dog)animal1;   //downcast - должны быть уверены что объект явлеятся тем типом к которому приводим
        dog1.run();

        Fish fish1 = new Fish();
        Animal animal2 = fish1;  //upcast
        //Dog dog2 = (Dog)animal2; //downcast - который не показывает ошибку, но мы упадем тк привели изначально не к тому типу


        System.out.println("Применение интерфейсов");
        Dog dog10 = new Dog();
        Dog dog20 = new Dog();
        Dog dog30 = new Dog();
        Dog dog40 = new Dog();
        ...
        Bird bird20 = new Bird();
        Bird bird30 = new Bird();
        Bird bird40 = new Bird();
        Bird bird50 = new Bird();
        //задача: в цикле заставить животных бегать
        //в типе если напишем Dog, то птиц не вставим, если вставим Bird, то собак не вставим
        //если напишем Animal то они не смогут бегать, что делать?
        //1. Один из вариантом добавить метод run() в Animal, но мы тогда наделим способностью бегать объекты класса Fish
        //2. Еще вариант создать класс который будет наследоваться от класса Animal и будет называться RunnubleAnimal и в нем
        //создать два абстрахтных метода eat(), run(), но тут проблема, а если Птица умеет бегать и летать, то нам нужно
        //опять создавать класс бегающие и летающие животные и мы так погрязнем в непонятных классах и не сможем их применить
        //тк в java запрещено множественное наследование классов.
        //3. Создать интерфейс - он опред. что умеет делать объект, по сути является абстрактным классом.
        //в интерфейсе метод run() не будет иметь параметров и не будет ничего возвращать
        ArrayList<CanRun> animals3 = new ArrayList<>();
        animals3.add(dog40);
        animals3.add(dog50);
        animals3.add(bird10);
        animals3.add(bird20);
        for (CanRun item: animals3) {
            item.run();
        }

        Dog dog200 = new Dog();
        CanRun canRun = dog200;
        canRun.run();   //и теперь у нашей собаки не будет доступен метод eat(), ну тоесть какой тип, туда и доступ
        dog200 = (Dog)canRun;
        dog200.eat();  //теперь доступ снова есть после downcast
